<?php
	...
	if($_GET['tgl'] == ''){
		$hariini = date('Y-m-d');
		$puskesmas_online = mysqli_num_rows(mysqli_query($koneksi,"SELECT * FROM `$tbpasienrj` where Kota = '$kota' and TglPs = curdate() and NamaPuskesmas <> 'DINAS KESEHATAN'  and NamaPuskesmas <> 'UPTD FARMASI'"));
	}else{
		$hariini = $_GET['tgl'];
		$puskesmas_online = mysqli_num_rows(mysqli_query($koneksi,"SELECT DISTINCT(substring(NoRegistrasi,1,11)) FROM `$tbpasienrj` where TanggalRegistrasi = '$hariini' and substring(NoRegistrasi,1,1)"));
	}
?>
...
<script>
	$(function() {
		...
		Morris.Bar({
			...
			data: [
			<?php
			$str = "SELECT `TanggalRegistrasi`, SUBSTRING(NoRegistrasi, 1, 11) as KodePuskesmas, COUNT(`NoRegistrasi`) as jml, kdprovider FROM 
			`$tbpasienrj` GROUP BY `TanggalRegistrasi`,`kodepuskesmas` HAVING `TanggalRegistrasi` = '$hariini' order by jml DESC";
				...
				<div class="table-responsive noprint">
					<table class="table table-condensed noprint">
						...
						<tbody style="font-size:11px;">
							<?php
							$no = 0;
							$query = mysqli_query($koneksi, "SELECT `TanggalRegistrasi`, SUBSTRING(NoRegistrasi, 1, 11) as KodePuskesmas, COUNT(`NoRegistrasi`) as jml, kdprovider 
							FROM `$tbpasienrj` GROUP BY `TanggalRegistrasi`,`kodepuskesmas` HAVING `TanggalRegistrasi` = '$hariini' order by jml DESC");
