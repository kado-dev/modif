			...
			<table class="table table-condensed" border="1">
				...
				<tbody>
					<?php						
						...
						$str2 = $str." GROUP BY KodeBarang, NamaProgram ORDER BY `IdProgram`,`NamaBarang` ASC LIMIT $mulai,$jumlah_perpage";
						// echo $str2;							
						...
						while($data = mysqli_fetch_assoc($query_obat)){
							...
							$namaprogran = $data['NamaProgram'];
							$nobatch = $data['NoBatch'];
							
							// Batch
							$no1 = 0;
							$query_batch = mysqli_query($koneksi, "SELECT `NoBatch` FROM `tbgfkstok` WHERE `KodeBarang`='$kodebarang' AND `IdProgram`='$idprogram'");
							...
							// Expire
							$no2 = 0;
							$query_expire = mysqli_query($koneksi, "SELECT `Expire` FROM `tbgfkstok` WHERE `KodeBarang`='$kodebarang' AND `IdProgram`='$idprogram'");
							...
							// Sumber Anggaran
							$no3 = 0;
							$query_sumber_anggaran = mysqli_query($koneksi, "SELECT `SumberAnggaran` FROM `tbgfkstok` WHERE `KodeBarang`='$kodebarang' AND `IdProgram`='$idprogram'");
							...
							WHERE a.`KodeBarang`='$kodebarang' AND a.`IdProgram`='$idprogram'");
							...
							$str_stokawal = "SELECT * FROM `tbstokawalmaster_gudang_besar` WHERE `KodeBarang`='$kodebarang' AND `NamaProgram`='$namaprogram'";
							// echo $str_stokawal;
