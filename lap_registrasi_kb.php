...
<div class="tableborderdiv">
	<div class="row">
		<div class="col-sm-12 table-responsive">
			<h3 class="judul"><b>REGISTER KB</b></h3>
			<div class="formbg">
				<form role="form">
					<div class="row">
						<input type="hidden" name="page" value="lap_registrasi_kb"/>
						<div class="col-xl-2">
						...
						<div class="col-xl-3">
							...
							<a href="?page=lap_registrasi_kb" class="btn btn-round btn-info"><span class="fa fa-refresh"></span></a>
							<a href="lap_registrasi_kb_excel.php?opsiform=<?php echo urlencode($opsiform);?>&keydate1=<?php echo urlencode($keydate1);?>&keydate2=<?php echo urlencode($keydate2);?>&bulan=<?php echo urlencode($bulan);?>&tahun=<?php echo urlencode($tahun);?>&kd=<?php echo urlencode($kodepuskesmas);?>" class="btn btn-round btn-success"><span class="fa fa-file-excel"></span></a>
	...
	<div class="printheader" style="display:none;">
		...
		<span class="font14"><b>REGISTER KB</b></span><br>
		...
		<div class="table-responsive noprint">
			<table class="table-judul-laporan" style="font-size:10px;">
				...
				<tbody>
					<?php
					// Query utama
					$str_main = "SELECT TanggalPeriksa, NoPemeriksaan, NoIndex, NoCM, NamaPasien, StatusPulang, NamaPegawaiSimpan
						FROM `tbpolikb`
					...
					if(!empty($rows)){
						...
						$q_rj = mysqli_query($koneksi, "SELECT NoRegistrasi, JenisKelamin, UmurTahun 
						...
						// Batch query tbpolikb_kunjunganulang
						$q_ku = mysqli_query($koneksi, "SELECT NoPemeriksaan, Anamnesa FROM `tbpolikb_kunjunganulang` WHERE NoPemeriksaan IN ($noreg_in)");
						if($q_ku){
							while($r = mysqli_fetch_assoc($q_ku)){
								$kb_ku_data[$r['NoPemeriksaan']] = $r;
							}
						}
					...
					foreach($rows as $data):
						...
						$pemeriksa = !empty($peg['NamaPegawai1']) ? $peg['NamaPegawai1'] : (isset($data['NamaPegawaiSimpan']) ? $data['NamaPegawaiSimpan'] : '');
						
						$ku = isset($kb_ku_data[$noregistrasi]) ? $kb_ku_data[$noregistrasi] : [];
						$anamnesa = isset($ku['Anamnesa']) ? $ku['Anamnesa'] : '';
					...
					?>
						<tr style="border:1px solid #000;">
							...
							<td><?php echo $umur;?></td>
							<td><?php echo $alamat_pasien != '' ? htmlspecialchars($alamat_pasien) : '<span style="color:red;">Belum Terdaftar</span>';?></td>
							<td><?php echo htmlspecialchars($anamnesa);?></td>
							...
							<td><?php echo strtoupper(htmlspecialchars($pemeriksa));?></td>
						</tr>
					<?php endforeach; 
					
					if(count($rows) == 0):
					?>
						<tr><td colspan="13" align="center">Tidak ada data</td></tr>
					<?php endif; ?>
				</tbody>
			</table>
		</div>
	...
	<div class="noprint">
		<?php if($h > 1): ?>
			<a href="?page=lap_registrasi_kb&opsiform=<?php echo urlencode($opsiform);?>&keydate1=<?php echo urlencode($keydate1);?>&keydate2=<?php echo urlencode($keydate2);?>&bulan=<?php echo urlencode($bulan);?>&tahun=<?php echo urlencode($tahun);?>&h=<?php echo ($h-1);?>" class="btn btn-info">&laquo; Prev</a>
		<?php else: ?>
		...
		<?php if($has_next): ?>
			<a href="?page=lap_registrasi_kb&opsiform=<?php echo urlencode($opsiform);?>&keydate1=<?php echo urlencode($keydate1);?>&keydate2=<?php echo urlencode($keydate2);?>&bulan=<?php echo urlencode($bulan);?>&tahun=<?php echo urlencode($tahun);?>&h=<?php echo ($h+1);?>" class="btn btn-info">Next &raquo;</a>
		<?php else: ?>
