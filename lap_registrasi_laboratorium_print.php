...
<body onload="window.print()" onafterprint="document.location = 'index.php?page=lap_registrasi_laboratorium'">
	<div class="printheader">
		...
		<span class="font11" style="margin:15px 5px 5px 5px;"><b>LAPORAN REGISTER PASIEN LABORATORIUM</b></span><br>
			...
			<table class="table table-condensed">
				<thead style="font-size:10px;">
					<tr style="border:1px dashed #000;">
						...
						<th rowspan="2" style="text-align:center;width:2%;vertical-align:middle; border:1px dashed #000; padding:3px;">Pemeriksaan</th>
						<th rowspan="2" style="text-align:center;width:2%;vertical-align:middle; border:1px dashed #000; padding:3px;">Hasil</th>
						<th colspan="2" style="text-align:center;width:1%;vertical-align:middle; border:1px dashed #000; padding:3px;">Rujuk</th>
						<th rowspan="2" style="text-align:center;width:1%;vertical-align:middle; border:1px dashed #000; padding:3px;">Jaminan</th>
				...
				<tbody style="font-size:10px;">
					<?php		
					$str = "SELECT * FROM `tbpolilaboratorium` a join `$tbpasienrj` b on a.NoPemeriksaan = b.NoRegistrasi 
					WHERE MONTH(a.TanggalPeriksa) = '$bulan' and YEAR(a.TanggalPeriksa) = '$tahun' 
					and substring(a.NoPemeriksaan,1,11) = '$kodepuskesmas' AND `PoliPertama` like '%Laboratorium%'";
					...
					$str_tdk = "SELECT `KodeTindakan` FROM `tbtindakanpasiendetail` WHERE `NoRegistrasi` = '$noregistrasi'";
					$query_tdk = mysqli_query($koneksi,$str_tdk);
					while($data_tindakan = mysqli_fetch_array($query_tdk)){
						$array_data[$no][] = $data_tindakan['KodeTindakan'];
					}
					if ($array_data[$no] != ''){
						$data_tdk = implode(",", $array_data[$no]);
					}else{
						$data_tdk ="";
					}
					?>
						...
						<tr style="border:1px dashed #000;">
							...
							<td style="text-align:left; border:1px dashed #000; padding:3px;"><?php echo $data_tdk;?></td>
							<td style="text-align:center; border:1px dashed #000; padding:3px;"><?php echo $hasil;?></td>
							...
							<td style="text-align:center; border:1px dashed #000; padding:3px;"><?php echo $asuransi;?></td>
