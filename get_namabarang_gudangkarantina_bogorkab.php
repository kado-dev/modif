...
error_reporting(E_ALL);
	...
	if($keyword != ''){
		if($kota == "KABUPATEN BOGOR"){
			$query = mysqli_query($koneksi,"SELECT * FROM `tbgfkstok` WHERE (`NamaBarang` LIKE'%$keyword%' OR `KodeBarang` LIKE'%$keyword%' OR `NoBatch` LIKE'%$keyword%') AND Stok > '0' ORDER BY `Expire`,`Stok` ASC");
