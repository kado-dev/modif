...
<div class="tableborderdiv">
	<div class="row noprint">
		<div class="col-xs-12">
			<h3 class="judul"><b>KETERSEDIAAN </b><small>Gudang Besar</small></h3>
			<div class="formbg">
				<form role="form">
					<div class = "row">
						<input type="hidden" name="page" value="uptd_gudang_sisa_aset_bekasikab"/>
						<div class="col-sm-2">
						...
						<div class="col-sm-3">
							<select name="sumberanggaran" class="form-control">
								...
								<option value="Semua Apbd" <?php if($_GET['sumberanggaran'] == 'Semua Apbd'){echo "SELECTED";}?>>SEMUA APBD</option>
								<option value="APBD OBAT PADAT" <?php if($_GET['sumberanggaran'] == 'APBD OBAT PADAT'){echo "SELECTED";}?>>APBD OBAT PADAT</option>
								<option value="APBD OBAT CAIR" <?php if($_GET['sumberanggaran'] == 'APBD OBAT CAIR'){echo "SELECTED";}?>>APBD OBAT CAIR</option>
								<option value="APBD OBAT GEL/SALEP" <?php if($_GET['sumberanggaran'] == 'APBD OBAT GEL/SALEP'){echo "SELECTED";}?>>APBD OBAT GEL/SALEP</option>
								<option value="APBD BAHAN OBAT LAINNYA (BMHP)" <?php if($_GET['sumberanggaran'] == 'APBD BAHAN OBAT LAINNYA (BMHP)'){echo "SELECTED";}?>>APBD BAHAN OBAT LAINNYA (BMHP)</option>
								<option value="APBD BAHAN OBAT LAINNYA (COVID)" <?php if($_GET['sumberanggaran'] == 'APBD BAHAN OBAT LAINNYA (COVID)'){echo "SELECTED";}?>>APBD BAHAN OBAT LAINNYA (COVID)</option>
								...
								<option value="BTT/DINKES" <?php if($_GET['sumberanggaran'] == 'BTT/DINKES'){echo "SELECTED";}?>>BTT/DINKES</option>
							</select>
						</div>	
						<div class="col-sm-2">
						...
						<div class="col-sm-3">
							...
							<a href="?page=uptd_gudang_sisa_aset_bekasikab" class="btn btn-sm btn-info"><span class="fa fa-refresh"></span></a>
							<a href="uptd_gudang_sisa_aset_bekasikab_excel.php?tanggal_awal=<?php echo $_GET['tanggal_awal'];?>&tanggal_akhir=<?php echo $_GET['tanggal_akhir'];?>&sumberanggaran=<?php echo $_GET['sumberanggaran'];?>&key=<?php echo $_GET['key'];?>" class="btn btn-sm btn-success"><span class="fa fa-file-excel"></span></a>
					...
					<tbody>
						<?php								
							...
							while($data = mysqli_fetch_assoc($query_obat)){
								...
								// WHERE a.`KodeBarang`='$kodebarang' AND a.`NoBatch`='$nobatch' AND b.TanggalKarantina < '$tanggal_awal'";
								...
								if($kota == "KABUPATEN BEKASI" AND $tanggalawal == "2021" AND $kodebarang == 'DRG00078' AND $nobatch == '2018a'){
								...
								AND b.TanggalPenerimaan BETWEEN '$tanggal_awal' AND '$tanggal_akhir'";
								// echo $strpenerimaan_vk;
								
								$strpenerimaan = "SELECT Jumlah FROM `tbgfk_vaksin_penerimaandetail` a 
								JOIN `tbgfk_vaksin_penerimaan` b ON a.NomorPembukuan = b. NomorPembukuan 
								WHERE a.`KodeBarang`='$kodebarang' AND a.`Nobatch`='$nobatch' AND a.`NomorPembukuan`='$nofakturterima' 
								AND b.TanggalPenerimaan BETWEEN '$tanggal_awal' AND '$tanggal_akhir'";
						...
						?>
							<tr>
								...
								<td style="text-align:left;">
									<?php 
										echo "<b>".strtoupper($data['NamaBarang'])."</b>,".$data['KodeBarang']."<br/>";
								...
								<td style="text-align:right;"><?php echo $stokawal_rupiah;?></td>
								<td style="text-align:right;"><?php echo rupiah($penerimaan);?></td>
								<td style="text-align:right;"><?php echo $penerimaan_rupiah;?></td>
