...
if(mysqli_num_rows($query ) > 0){
	...
	//cek formulir skrining berdasarkan siklus hidup yg diisi di registrasi
	$jmlformulir = 0;
	$siklushidup = $data['SiklusHidup'];
	$qryskh = mysqli_query($koneksi, "SELECT `IdSiklusHidup` FROM `ref_siklushidup` WHERE `Nama` = '$siklushidup'");
	if(mysqli_num_rows($qryskh ) > 0){
		$dts = mysqli_fetch_assoc($qryskh);
		$jmlformulir = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT count(`IdSiklusHidupSkrining`) AS JmlFoormulir FROM `siklushidup_skrining` WHERE `IdSiklusHidup` = '$dts[IdSiklusHidup]'"));
	}
											...
											<div class="text-medium text-uppercase fw-bold op-8">
												<?php 
													if($datapasien['NoAsuransi'] == null || $datapasien['NoAsuransi'] == 0){ 
														$noasuransi = "<span style='color:red;font-weight:bold'>Belum Terdaftar</span>";
													}else{ 
													...
													if($datakk['Telepon'] != ''){
														...
														if($datapasien['Telpon'] != ''){
															...
															$telp = "<span style='color:red;font-weight:bold'>Belum Diinputkan</span>";
														}	
													}	
													...
													$datakk['Alamat'];?> RT.<?php echo $datakk['RT'];?> NO.<?php echo $datakk['No'];?> <?php echo $datakk['Kelurahan']."<br/>".
													$telp;
												?>
											</div>
										</div>
						...
						<div class="col-sm-3">
							<div class="card card-dark bg-primary-gradient">
								<div class="card-body skew-shadow">
									...
									<br/>
									<a href='?page=suratketerangan&idrj=<?php echo $idpasienrj;?>' target="_blank" style="color:blue !important">
										<div class="btn btn-round btn-xs btn-info btninfo" style="font-size: 12px; margin-top: -5px;"> Tampilkan Formulir</div>
									</a>
								</div>
							</div>				
						</div>
						...
						<div class="col-sm-3">
							<div class="card card-dark bg-success-gradient">
								<div class="card-body skew-shadow">
									...
									<br/>
									<a href='?page=skrining&idrj=<?php echo $idpasienrj;?>&idsh=<?php echo $dts['IdSiklusHidup'];?>' target="_blank" style="color:blue !important">
			...
			<div class="table-responsive">
				<table class="table-judul" width="100%">
					...
					while($riwayat = mysqli_fetch_assoc($queryriwayat)){
						...
						// pilih pelayanan
						if ($riwayat['PoliPertama'] == 'POLI GIGI'){
							$tbpoli = "tbpoligigi_".str_replace(' ', '', $namapuskesmas);
						}elseif($riwayat['PoliPertama'] == 'POLI KIA'){
							$tbpoli = "tbpolikia_".str_replace(' ', '', $namapuskesmas);
						}elseif($riwayat['PoliPertama'] == 'POLI LANSIA'){
							$tbpoli = "tbpolilansia_".str_replace(' ', '', $namapuskesmas);
						}elseif($riwayat['PoliPertama'] == 'POLI MTBS'){
							$tbpoli = "tbpolimtbs_".str_replace(' ', '', $namapuskesmas);
						}elseif($riwayat['PoliPertama'] == 'POLI UMUM'){
							$tbpoli = "tbpoliumum_".str_replace(' ', '', $namapuskesmas);
						}elseif($riwayat['PoliPertama'] == 'POLI KB'){
							$tbpoli = "tbpolikb_kunjunganulang";
						}else{
							$tbpoli = "tb".strtolower(str_replace(' ', '', $pel));
						}																				

						// cek pelayanan
						$pelayanan = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT `Anamnesa`,`Keluhan` FROM `$tbpoli` WHERE IdPasienrj = '$idpasienrj_riwayat'"));
						?>
						...
						<div class="formbg" style="background: #ffe1bc;">
							<p style="line-height: 16px; font-size:12px;">                                    
								...
								<b>Anamnesa :</b>
								<?php echo $pelayanan['Anamnesa'];?><br/>
								<b>Keluhan :</b>
								<?php echo $pelayanan['Keluhan'];?><br/>
								<b>Vitalsign</b><br/>
								<?php 
									// vital sign                                                
	...
	<script type="text/javascript">
		$(document).ready(function() {	
			...
			$('.btnmodalriwayat').click(function(){
				...
				alert(noregistrasi);
				$.post( "get_modal_riwayat.php", { no: noregistrasi})
				.done(function( data ) {
