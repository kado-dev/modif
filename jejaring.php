<?php
	...
	// get kodepelayanan
	$dtpelayanan = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT * FROM `tbasalpasien` WHERE `AsalPasien`='$pel'"));
	$kdpelayanan = $dtpelayanan['Id'];
?>
	...
	<div class="page-inner py-5">
		<div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
			...
			<div class="ml-md-auto py-2 py-md-0">
				<a href="index.php?page=jejaring&pelayanan=<?php echo $pel?>&status=Antri&tptgl=No" class="btn btn-danger btn-round"><?php echo "Belum di entry bulan ini : ".$dt_antri['Jml']." Pasien";?></a>
			</div>
		</div>
	</div>
</div>
			...
			<div class="card">
				<div class="card-body">
					<form role="form" class="submit">
						<div class="row">	
							<input type="hidden" name="page" value="jejaring"/>
							...
							<div class="col-xl-3">
								<input type="text" name="nama" class="form-control inputan" value="<?php echo $_GET['nama'];?>" placeholder = "Masukan Nama Pasien">
							</div>
							<div class="col-xl-2">
									...
									$query = mysqli_query($koneksi,"SELECT * FROM `tbpelayanankesehatan` WHERE `JenisPelayanan` = 'KUNJUNGAN SAKIT' ORDER BY `Pelayanan`");
										while($data = mysqli_fetch_assoc($query)){											
											if($data['Pelayanan'] == $_GET['polipertama']){
												echo "<option class='tmp_pelayanan' value='$data[Pelayanan]' SELECTED>$data[Pelayanan]</option>";
											}else{
												echo "<option class='tmp_pelayanan' value='$data[Pelayanan]'>$data[Pelayanan]</option>";
											}
										}
									?>
								</select>
							...
							<div class="col-xl-2">
								<select name="status" class="form-control inputan">
							...
							<div class="col-xl-3">
								...
								<a href="?page=jejaring&pelayanan=<?php echo $pel;?>&status=Antri" class="btn btn-round btn-info"><span class="fa fa-refresh"></span></a>
			...
			<div class="table-responsive">
				<table class="table-judul">
					<thead>
						...
						<tr>
							<th width="3%">NO.</th>
							<th><a href="?page=jejaring&pelayanan=<?php echo $pel;?>&tgl=<?php echo $_GET['tgl'];?>&nama=<?php echo $_GET['nama'];?>&status=<?php echo $_GET['status'];?>&tptgl=<?php echo $_GET['tptgl'];?>&orderby=a.IdPasienrj&sort=<?php echo $sorts;?>&h=<?php echo $_GET['h'];?>">TGL.DAFTAR <?php echo iconsort("a.IdPasienrj",$sorts);?></a></th>
							<th>NO.INDEX</th>
							<th><a href="?page=jejaring&pelayanan=<?php echo $pel;?>&tgl=<?php echo $_GET['tgl'];?>&nama=<?php echo $_GET['nama'];?>&status=<?php echo $_GET['status'];?>&tptgl=<?php echo $_GET['tptgl'];?>&orderby=a.NamaPasien&sort=<?php echo $sorts;?>&h=<?php echo $_GET['h'];?>">NAMA PASIEN <?php echo iconsort("a.NamaPasien",$sorts);?></a></th>
							<th>L/P</th>
							<th>UMUR</th>
							<th>CARA BAYAR</th>
							<th>STATUS</th>
					...
					if($status == 'Antri'){		
						...
						WHERE ".$tgl_str." AND `StatusPelayanan` = 'Antri' AND `AsalPasien`='$kdpelayanan'".$nama_str;
						...
						<tr class="btnprint" data-pel="<?php echo $data['PoliPertama'];?>" data-noreg="<?php echo $data['NoRegistrasi'];?>" data-idrj="<?php echo $data['IdPasienrj'];?>">
							...
							<td style="text-align:left;">
								...
								<?php
									...
									?>	
										<a href="?page=poli_periksa&id=<?php echo $data['IdPasienrj'];?>&idps=<?php echo $data['IdPasien'];?>&pelayanan=<?php echo $polipertama;?>&stspelayanan=<?php echo $pel;?>&status=<?php echo $status;?>&tptgl=<?php echo $tglskrg;?>&cb=<?php echo $data['Asuransi'];?>" class="btn btn-round btn-success"><i class="fa fa-user-md (alias) faicon"></i></a>
									<?php 
									}
									?>	
									<?php
					...
					<?php
						...
						if($polipertama == 'POLI GIGI'){
							...
							$tbpelayanan = "tb".strtolower(str_replace(' ','', $polipertama));
						}
						...
						while($datapel = mysqli_fetch_assoc($querypel)){
							...
							// noantrian
							$dtpasienrj = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT * FROM `$tbpasienrj` WHERE `NoRegistrasi` = '$nopemeriksaan' AND `NoIndex`='$noindex'"));
					...
					?>
						<tr>
							...
							<td style="text-align:center;">
								...
								<?php } ?>
								<a href="?page=poli_periksa_edit&id=<?php echo $dtpasienrj['IdPasienrj'];?>&idpr=<?php echo $datapel['IdPemeriksaan']?>&idps=<?php echo $dtpasienrj['IdPasien'];?>&pelayanan=<?php echo $polipertama;?>&cb=<?php echo $dtpasienrj['Asuransi'];?>&sts=ukm" class="btn btn-round btn-success"> EDIT</a>
							</td>
						</tr>
					<?php
						}
				...
				if($i <= $max && $i >= $min){
					if($_GET['h'] == $i){
						...
						if($_GET['status'] == 'Antri'){
							if($tptgl == ''){
								echo "<li><a href='?page=jejaring&pelayanan=$pel&status=$status&tgl=$tglskrg&h=$i&orderby=$orderbys&sort=$sorts'>$i</a></li>";
							}else{	
								echo "<li><a href='?page=jejaring&pelayanan=$pel&status=$status&tgl=$tglskrg&h=$i&tptgl=$tptgl&orderby=$orderbys&sort=$sorts'>$i</a></li>";
							}
						}else{
							echo "<li><a href='?page=jejaring&pelayanan=$pel&tgl=$tgl&nama=$nama&status=Sudah&h=$i&tptgl=$tptgl&orderby=$orderbys&sort=$sorts'>$i</a></li>";
						}
					}
				}
			}
