		...
		<div class="page">
			...
			if($statusprint == 'Etiket'){
				...
				if($idpasienrj == ''){
					...
					$str = "UPDATE `$tbresep` SET `Status`='Sudah', `Pio`='$jenis_pio', `Telaah`='$jenis_telaah', `StatusKonseling`='$statuskonseling', `NamaPegawai`='$tenagafarmasi' WHERE `IdPasienrj`='$idpasienrj'";
				}
				...
				<tr>
					<td colspan="2">						
						<table>
							...
							if($idpasienrj == ''){
								$cekrsp = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM `$tbresep` WHERE `NoResep`='$norsp' AND `Pelayanan`='$pelayanan'"));
					...
					<table class="tablehead">
						<!--<td width="25%">
							...
							<tr>
								...
								<?php 
									if($data_resep['Pelayanan'] == "POLI UMUM"){
