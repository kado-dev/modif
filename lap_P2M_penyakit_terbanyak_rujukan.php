...
<div class="tableborderdiv">
	<div class="row noprint">
		<div class="col-xs-12">
			<h3 class="judul"><b>PENYAKIT TERBANYAK (RUJUKAN)</b></h3>
			<div class="formbg">
				<div class = "row">
					<form role="form">
						<input type="hidden" name="page" value="lap_P2M_penyakit_terbanyak_rujukan"/>
						...
						<div class="col-sm-2">
							...
							<a href="?page=lap_P2M_penyakit_terbanyak_rujukan" class="btn btn-sm btn-success"><span class="fa fa-refresh"></span></a>
	...
	<div class="table-responsive printini" style="overflow: hidden;">
		<div class="printheader">
			...
			<span class="font11" style="margin:15px 5px 5px 5px;"><b>LAPORAN PENYAKIT TERBANYAK (RUJUKAN)</b></span><br>
				...
				<table class="table-judul-laporan-min" width="100%">
					...
					<tbody style="font-size:10px;">
						...
						$str = "SELECT a.KodeDiagnosa, a.Kasus as Kasus, COUNT(a.KodeDiagnosa) AS Jumlah 
								...
								WHERE SUBSTRING(a.NoRegistrasi,1,11)='$kodepuskesmas' AND a.KodeDiagnosa <> 'Z00.0' AND YEAR(a.TanggalDiagnosa) = '$tahun' AND `StatusPulang`='4'
						...
						while($data = mysqli_fetch_assoc($query)){
							...
							if($_SESSION['kodepuskesmas'] == '-'){
								$jml_laki = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT COUNT(a.NoRegistrasi)AS Jumlah 
										FROM `$tbdiagnosapasien` a 
										LEFT JOIN `$tbpasienrj` b on a.NoRegistrasi = b.NoRegistrasi
										WHERE ".$kodepuskesmas." YEAR(a.TanggalDiagnosa)='$tahun' AND a.KodeDiagnosa = '$kodediagnosa' AND b.JenisKelamin = 'L' AND `StatusPulang`='4'"));
								$jml_perempuan = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT COUNT(a.NoRegistrasi)AS Jumlah 
										FROM `$tbdiagnosapasien` a 
										LEFT JOIN `$tbpasienrj` b on a.NoRegistrasi = b.NoRegistrasi
										WHERE ".$kodepuskesmas." YEAR(a.TanggalDiagnosa)='$tahun' AND a.KodeDiagnosa = '$kodediagnosa' AND b.JenisKelamin = 'P' AND `StatusPulang`='4'"));
							}else{
								$jml_laki = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT COUNT(a.NoRegistrasi)AS Jumlah 
										FROM `$tbdiagnosapasien` a 
										LEFT JOIN `$tbpasienrj` b on a.NoRegistrasi = b.NoRegistrasi
										WHERE SUBSTRING(a.NoRegistrasi,1,11) = '$kodepuskesmas' AND YEAR(a.TanggalDiagnosa)='$tahun' AND a.KodeDiagnosa = '$kodediagnosa' AND b.JenisKelamin = 'L' AND `StatusPulang`='4'")); 
								$jml_perempuan = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT COUNT(a.NoRegistrasi)AS Jumlah 
										FROM `$tbdiagnosapasien` a 
										LEFT JOIN `$tbpasienrj` b on a.NoRegistrasi = b.NoRegistrasi
										WHERE SUBSTRING(a.NoRegistrasi,1,11) = '$kodepuskesmas' AND YEAR(a.TanggalDiagnosa)='$tahun' AND a.KodeDiagnosa = '$kodediagnosa' AND b.JenisKelamin = 'P' AND `StatusPulang`='4'"));
							}
	...
	<?php
		...
		FROM `$tbdiagnosapasien` a 
		LEFT JOIN `$tbpasienrj` b ON a.NoRegistrasi = b.NoRegistrasi
		WHERE SUBSTRING(a.NoRegistrasi,1,11)='$kodepuskesmas' AND YEAR(a.TanggalDiagnosa) = '$tahun' AND a.KodeDiagnosa <> 'Z00.0' AND `StatusPulang`='4'
		GROUP BY KodeDiagnosa 
		ORDER BY Jumlah DESC
		LIMIT 0,10";
	?>
