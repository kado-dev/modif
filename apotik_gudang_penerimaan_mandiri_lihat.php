...
<div class="tableborderdiv noprint">
	...
	<div class="row noprint">
		<div class="col-lg-12">
			<div class="formbg">
				<div class="table-responsive">
					<form action="?page=apotik_gudang_penerimaan_mandiri_lihat_proses" method="post">
						<table class="table-judul" width="100%">
							<tr>
								<td class="col-sm-4">
								...
								<td colspan="1"><input type="submit" value="SIMPAN" class="btn btn-round btn-success pull-right"></td>
								<?php }?>
				...
				<table class="table-judul-laporan" width="100%">
					...
					<tbody>
						...
						while($datapenerimaan = mysqli_fetch_assoc($query)){
							$no = $no + 1;
							// tbgfkstok`
							$dt_gfkstok = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT `NamaBarang`,`Satuan` FROM `$tbgudangpkmstok` WHERE `KodeBarang` = '$datapenerimaan[KodeBarang]'"));
						
						?>
							<tr>
								<td align="center"><?php echo $no;?></td>
								<td align="center"><?php echo $datapenerimaan['KodeBarang'];?></td>
								<td align="left" class="nama"><?php echo strtoupper($dt_gfkstok['NamaBarang']);?></td>
								<td align="center"><?php echo strtoupper($dt_gfkstok['Satuan']);?></td>
								...
								<?php if (in_array("APOTEK", $otoritas) || in_array("ADMINISTRATOR", $otoritas) || in_array("PROGRAMMER", $otoritas)){?>
								<td align="center">
									<a href="?page=apotik_gudang_penerimaan_mandiri_lihat_hapus&kd=<?php echo $datapenerimaan['KodeBarang']?>&no=<?php echo $nf;?>&bat=<?php echo $datapenerimaan['NoBatch'];?>" class="btn btn-round btn-sm btn-danger btnhapus">HAPUS</a>
								</td>
								<?php }?>							
							</tr>
						<?php
	...
	<table>
		...
		<tbody>
			...
			while($data = mysqli_fetch_assoc($query_print)){
				...
				$data_gop=mysqli_fetch_assoc(mysqli_query($koneksi,"SELECT * FROM `tbgudangpkmstok` WHERE `KodeBarang` = '$kodebarang' and `KodePuskesmas` = '$kodepuskesmas'"));
