...
<div class="tableborderdiv">
	<div class="row noprint">
		<div class="col-xs-12">
			<h3 class="judul"><b>PENYAKIT TERBANYAK (KUNJUNGAN)</b></h3>
			<div class="formbg">
				<div class = "row">
					<form role="form">
						<input type="hidden" name="page" value="lap_P2M_penyakit_terbanyak_kunjungan"/>
						...
						<div class="col-sm-3">
							<select name="asalpasien" class="form-control" required>
								<option value="Semua">--Semua--</option>
								<?php
								$query = mysqli_query($koneksi,"SELECT * FROM `tbasalpasien` order by `AsalPasien`");
									while($data = mysqli_fetch_assoc($query)){
										if($data['Id'] == $_GET['asalpasien']){
						...
						<div class="col-sm-2">
							...
							<a href="?page=lap_P2M_penyakit_terbanyak_kunjungan" class="btn btn-sm btn-success"><span class="fa fa-refresh"></span></a>
				...
				<table class="table-judul-laporan-min" width="100%">
					...
					<tbody style="font-size:10px;">
						...
						if($asalpasien == 'Semua'){
							...
							$asalpasiens = "  AND b.AsalPasien = '$asalpasien'";
						}	
						...
						$str = "SELECT a.KodeDiagnosa, a.Kasus as Kasus, COUNT(a.KodeDiagnosa) AS Jumlah 
								...
								WHERE SUBSTRING(a.NoRegistrasi,1,11)='$kodepuskesmas' AND YEAR(a.TanggalDiagnosa) = '$tahun' AND a.KodeDiagnosa <> 'Z00.0'".$asalpasiens."
						...
						while($data = mysqli_fetch_assoc($query)){
							...
							if($_SESSION['kodepuskesmas'] == '-'){
								$jml_laki = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT COUNT(a.NoRegistrasi)AS Jumlah 
										...
										and a.KodeDiagnosa = '$kodediagnosa' and b.JenisKelamin = 'L'".$asalpasiens));
								$jml_perempuan = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT COUNT(a.NoRegistrasi)AS Jumlah 
										FROM `$tbdiagnosapasien` a 
										LEFT JOIN `$tbpasienrj` b on a.NoRegistrasi = b.NoRegistrasi
										WHERE ".$kodepuskesmas." YEAR(a.TanggalDiagnosa)='$tahun' and MONTH(a.TanggalDiagnosa)='$bulan'
										and a.KodeDiagnosa = '$kodediagnosa' and b.JenisKelamin = 'P'".$asalpasiens));
							}else{
								$jml_laki = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT COUNT(a.NoRegistrasi)AS Jumlah 
										FROM `$tbdiagnosapasien` a 
										LEFT JOIN `$tbpasienrj` b on a.NoRegistrasi = b.NoRegistrasi
