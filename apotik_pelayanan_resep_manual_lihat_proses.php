<?php
	...
	foreach($kodebarang as $kode){
		if($kode != ""){
			...
			if($cek['Jml'] != 0){
				...
				echo "document.location.href='index.php?page=apotik_pelayanan_resep_manual_lihat&status=&norsp=$noresep&noid=$noindex&statusloket=$statusloket';";
				echo "</script>";
				die();
			}		
			...
			$strrsp = "INSERT INTO `$tbresepdetail`(`TanggalResep`,`NoResep`,`IdPasienrj`,`racikan`,`kdRacikan`,`obatDPHO`,`KodeBarang`,`NoBatch`,`signa1`,`signa2`,`jumlahobat`,`jmlPermintaan`,`nmObatNonDPHO`,`KdObatSk`,`Pelayanan`,`AnjuranResep`,`KeteranganRacikan`,`Depot`) 
			VALUES ('$tanggalresep','$noresep','$idpasienrj','$racikan[$i]','$kdRacikan','$namaobatbpjs[$i]','$kodebarang[$i]','$nobatch[$i]','$signa1[$i]','$signa2[$i]','$jumlah[$i]','$jmlpermintaan','$namaobatnondpho[$i]','$kdobatsk2','$pelayanan','$anjuran_s','$ket_racikan[$i]','$statusloket')";
	...
	if($queryrsp){	
		...
		if($idpasienrj == '0'){
			echo "document.location.href='index.php?page=apotik_pelayanan_resep_manual_lihat_offline&status=&idrj=$idpasienrj&norsp=$noresep&noid=$noindex&statusloket=$statusloket';";
		}else{
			echo "document.location.href='index.php?page=apotik_pelayanan_resep_manual_lihat&status=&idrj=$idpasienrj&norsp=$noresep&noid=$noindex&statusloket=$statusloket';";
		}
		echo "</script>";
	}else{
		echo "<script>";
