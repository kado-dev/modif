...
if($pelayanan == 'POLI UMUM'){
	...
	$str = "INSERT INTO `$tbpoliumum`(`TanggalPeriksa`, `JamPeriksa`, `NoPemeriksaan`, `NoIndex`, `NoCM`, `NamaPasien`,
	...
	mysqli_query($koneksi, "DELETE FROM `tbpoligigi` WHERE `NoPemeriksaan`='$nopemeriksaan' AND `NoCM`='$nocm'");
	$str = "INSERT INTO `tbpoligigi`(`TanggalPeriksa`,`JamPeriksa`,`NoPemeriksaan`,`NoIndex`,`NoCM`,`NamaPasien`,`JenisKelamin`,
	`Anamnesa`,`Anjuran`,`PemeriksaanHasilLab`,`PemeriksaanPenunjang`,`Sistole`,`Diastole`,`SuhuTubuh`,`BeratBadan`,`TinggiBadan`,
	`DetakNadi`,`RR`,`Kesadaran`,`RencanaTerapi`,`InformedConsent`,`Tindakan`,`Palpasi`,`SuhuKulit`,`Bibir`,`KelenjarLinfe`,
	...
	`KunjunganGigi`,`TerimaRujukan`,`Terapi`,`Diagnosa`,`NoResep`,`StatusPulang`,`RujukInternal`,`RujukLanjut`,`NamaPegawaiSimpan`) 
	VALUES ('$tanggal_registrasi','$jamperiksa','$nopemeriksaan','$noindex','$nocm',
	'$namapasien','$kelaminpasien','$anamnesa','$anjuran','$pemeriksaanhasillab','$pemeriksaanpenunjang','$sistole','$diastole','$suhutubuh','$beratbadan',
	'$tinggibadan','$heartrate','$resprate','$kesadaran','$rencanaterapi','$informedconsent','$tindakangigi',
	...
	foreach($nogigi_odontogram as $nogigi => $val_odon){
		mysqli_query($koneksi, "INSERT INTO `tbpoligigi_odontogram`(`IdPasien`, `NoGigi`, `IdOdontogram`) VALUES ('$idpasien','$nogigi','$val_odon')");
	}
	...
	`FaktorResikoDesc`,`KomplikasiDitanganiIbuHamil`,`JikaRujuk`,`Gravida`,`Partus`,`Abortus`,`Hpht`,`Djj`,`KepThd`,`Tbj`,`JumlahJanin`,`Presentasi`,`P4K`,`InjeksiTT`,
	...
	'$deteksi_resiko','$faktorresiko_kia','$faktorresikodesc_kia','$komplikasi_kia','$rujuk_komplikasi','$gravida','$partus','$abortus','$hpht_kia','$djj_kia','$kepalathd_kia','$tbj_kia',
	...
	foreach($arrykdtindakan as $kd){
		...
		if($cekinsertbtindakandetail == 0){
			...
			$str2 = "INSERT INTO `$tbtindakanpasien`(`TanggalTindakan`,`NoRegistrasi`,`NoIndex`,`NoCM`,`NamaPasien`,`UmurTahun`,`UmurBulan`,`UmurHari`,`JenisKelamin`,`PoliAsal`,`IdTindakan`,`KelompokTindakan`,`JenisTindakan`,`Keterangan`,`StatusBayar`,`NamaPegawaiSimpan`) 
				VALUES ('$tanggal_registrasi','$noregistrasi','$noindex','$nocm','$namapasien','$umurtahunpasien','$umurbulanpasien','$umurharipasien','$kelaminpasien','$pelayanan','$kd','$kelompoktindakan','$jenistindakan','$hasilkdtindakan2','BELUM','$tenagamedisbpjs')";
		}else{
	...
	if($statuspulang == 5){
		...
		mysqli_query($koneksi, "UPDATE `$tbpasienrj` SET `StatusPelayanan` = 'Sudah', `StatusPulang` = '$statuspulang', `IdConditionSatuSehat` = '$id_cond_satusehat', `IdObservationSatuSehat` = '$id_obs_satusehat' WHERE `IdPasienrj` = '$idpasienrj'");	
	...
	// inset tindakan
	$no = -1;
	mysqli_query($koneksi, "DELETE FROM `$tbtindakanpasien` WHERE `NoRegistrasi`='$nopemeriksaan' AND `NoCM`='$nocm'");
	...
	foreach($idtindakanbpjs as $kdtindakan){
		...
		$str_tindakanpasiendetail = "INSERT INTO `$tbtindakanpasien`(`TanggalTindakan`,`NoRegistrasi`,`NoIndex`,`NoCM`,`NamaPasien`,`PoliAsal`,`CaraBayar`,`IdTindakan`,`Tarif`,`KategoriTindakan`,`Keterangan`,`StatusBayar`,`NamaPegawaiSimpan`) 
		VALUES ('$tanggal_time','$nopemeriksaan','$noindex','$nocm','$namapasien','$pelayanan','$asuransi','$idtindakanbpjs[$no]','$tariftindakanbpjs[$no]','$kattindakan','$keteranganbpjs[$no]','BELUM','$tenagamedisbpjs')";
	...
	if(substr($asuransi,0,4) == 'BPJS'){
		if($kodepuskesmas == 'P3202280201' OR $kodepuskesmas == 'P3202180201' OR $kodepuskesmas == 'P3202230101'){
			...
			// update tbpasienrj
			$strupdatenokunjungan = "UPDATE `$tbpasienrj` 
			SET `NoKunjunganBpjs`= '$no_kunjungan', `KeteranganBridging` = '$keteranganbridging',
			`StatusBuku` = 'i', `JamKembaliRM`='$tanggal_time' 
			WHERE `NoRegistrasi` = '$nopemeriksaan'";
	...
	$strupdatenokunjungan = "UPDATE `$tbpasienrj` 
	SET `StatusBuku` = 'i', `JamKembaliRM`='$tanggal_time' 
	WHERE `NoRegistrasi` = '$nopemeriksaan'";
	mysqli_query($koneksi, $strupdatenokunjungan);	
	...
	// if($opsiresep == 'konseling' OR $opsiresep == 'resep luar'){
