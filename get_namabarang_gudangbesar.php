...
error_reporting(E_ALL);
	...
	if($keyword != ''){
		if($kota == "KABUPATEN BOGOR"){
			$query = mysqli_query($koneksi,"SELECT * FROM `tbgfkstok` WHERE (`NamaBarang` LIKE'%$keyword%' OR `KodeBarang` LIKE'%$keyword%' OR `NoBatch` LIKE'%$keyword%') AND Stok > '0' AND `Expire` > curdate() AND (`NamaProgram`='OBAT-OBATAN' OR `NamaProgram`='BMHP')
			...
			$query = mysqli_query($koneksi,"SELECT * FROM `tbgfkstok` WHERE (`NamaBarang` LIKE'%$keyword%' OR `KodeBarang` LIKE'%$keyword%' OR `NoBatch` LIKE'%$keyword%') AND Stok > '0' AND `Expire` > curdate() AND `SumberAnggaran`!= 'BLUD' GROUP BY `KodeBarang`, `NoBatch` ORDER BY `NamaBarang`,`Expire` ASC");
		}	
