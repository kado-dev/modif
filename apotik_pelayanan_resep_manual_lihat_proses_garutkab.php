<?php
	...
	foreach($kodebarang as $kode){
		if($kode != ""){
			...
			$strrsp = "INSERT INTO `$tbresepdetail`(`NoResep`,`racikan`,`kdRacikan`,`obatDPHO`,`KodeBarang`,`NoBatch`,`signa1`,`signa2`,`jumlahobat`,`jmlPermintaan`,`nmObatNonDPHO`,`KdObatSk`,`Pelayanan`,`AnjuranResep`,`KeteranganRacikan`) VALUES ('$noresep','$racikan[$i]','$kdRacikan','$obatdphao','$kodebarang[$i]','$nobatch[$i]','$signa1[$i]','$signa2[$i]','$jumlah[$i]','$jmlpermintaan','$namaobatnondpho','$kdobatsk2','$pelayanan','$anjuran_s','$ket_racikan[$i]')";
			$queryrsp=mysqli_query($koneksi,$strrsp);
			
			// update stok tbapotikstok
			$get_stok_lama = mysqli_fetch_assoc(mysqli_query($koneksi,"SELECT `Stok` FROM `tbapotikstok` WHERE `KodeBarang`='$kodebarang[$i]' AND `NoBatch`='$nobatch[$i]' AND `KodePuskesmas`='$kodepuskesmas'"));
			$stok_baru = $get_stok_lama['Stok'] - $jumlah[$i];			
			$str_obat_update = "UPDATE `tbapotikstok` SET `Stok`='$stok_baru' WHERE `KodeBarang`='$kodebarang[$i]' AND `NoBatch`='$nobatch[$i]' AND `KodePuskesmas`='$kodepuskesmas'";
	...
	if($queryrsp){	
		...
		echo "document.location.href='index.php?page=apotik_pelayanan_resep_manual_lihat_garutkab&status=&norsp=$noresep';";
		echo "</script>";
	}else{
		echo "<script>";
