<?php
	...
	if($sts == 'selesai'){
		...
		mysqli_query($koneksi,"UPDATE `tbantrian_view` SET `DisplayUtama`= '', `$disp`= '$nomorantrian', `Waktu` = '$time' WHERE `KodePuskesmas`='$kodepuskesmas'");
		//display antiran
		...
		$time = date('Y-m-d H:i:s');
		mysqli_query($koneksi,"UPDATE `tbantrian_view` SET `DisplayUtama`= '', `Waktu` = '$time' where `KodePuskesmas` = '$kodepuskesmas' ");
		//display antiran
		...
		//mencari selisih
		$dtwaktu = mysqli_fetch_assoc(mysqli_query($koneksi,"SELECT `Waktu` from `tbantrian_view` where `KodePuskesmas` = '$kodepuskesmas'"));
		$selisih = strtotime($time) - strtotime($dtwaktu['Waktu']);

		//jika lebih dari 30 detik baru panggil lagi/yg baru
		if($selisih > 3){
			mysqli_query($koneksi,"UPDATE `tbantrian_view` SET `$disp`='$nomorantrian',`DisplayUtama`= '$nomorantrian|$loket', `Waktu` = '$time' where `KodePuskesmas` = '$kodepuskesmas'"); //AND `DisplayUtama` = ''(query yg lama)
		}
		echo $selisih;
