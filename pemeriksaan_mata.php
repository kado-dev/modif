<?php
    ...
    if($_POST['btnsimpan'] == 'simpan'){
        ...
        $pelayanan = $dataregistrasi['PoliPertama'];       

        // tbsurat
        $str = "INSERT INTO `$tbsuratmata`(`IdPasienrj`, `IdPasien`, `TanggalPeriksa`, `NomorSurat`, `StatusPemeriksaan`, `Keperluan`, `Catatan`, `NamaDokter`) 
        VALUES ('$idpasienrj','$idpasien','$tanggalawal','$nomorsurat','$statuspemeriksaan','$keperluan','$catatan','$namadokter')";
        // echo $str;
        //die();
        ...
        // update nomorsurat
        $nomorsurat = $idsurat."/".$dtpuskesmas['NomorSuratMata'];    
         mysqli_query($koneksi, "UPDATE $tbsuratmata SET `NomorSurat`='$nomorsurat' WHERE `IdSuratSakit`='$idsurat'");

        if($query){
            alert_swal('sukses','Data berhasil disimpan..');
            echo "<script>";
            echo "document.location.href='index.php?page=pemeriksaan_mata&idrj=$idpasienrj';";	
            echo "</script>";
        }else{
...
<div class="tableborderdiv">
    <form action="pemeriksaan_mata.php" method="post">
        ...
        <div class = "row">
            <div class="col-sm-12">
                <div class="formbg mt-2">
                    ...
                    <p style="font-size: 18px; font-weight: bold;" class="judul">Pemeriksaan Mata</p>
                    <div class="table-responsive">
                        <table class="table-konten" width="100%">
                            <tr>
                                ...
                                <td class="col-sm-10">
                                    <div class="btn-group btn-block">
                                        <input type="text" name="nomorsurat" class="form-control inputan" value="<?php echo $dtpuskesmas['NomorSuratMata'];?>" readonly>
                            ...
                            <tr>
                                <td>Tanggal Awal</td>
                                <td>
                                    <input type="text" name="tanggalawal" class="form-control inputan datepicker tglreg" value="<?php echo $hariini;?>" autofocus>
                                </td>
                            </tr>
                            <tr>
                                <td>Penglihatan</td>
                                <td>
                                    <select name="statuspemeriksaan" class="form-control inputan" required>
                            ...
                            <tr>
                                <td>Untuk Kepeluan</td>
                                <td><input type="text" name="keperluan" class="form-control inputan" placeholder="Jelaskan"></td>
                            </tr>
                            <tr>
                                <td>Catatan</td>
                                <td><input type="text" name="catatan" class="form-control inputan"  placeholder="Jelaskan"></td>
                            </tr>
                            <tr>
                                <td>Dokter</td>
                                <td>
            ...
            <div class="table-responsive">
                <table class="table-judul">
                    ...
                    <tbody>
                        ...
                        $str = "SELECT * FROM `$tbsuratmata` WHERE IdPasien = '$idpasien'";
                        ...
                        while($data = mysqli_fetch_assoc($query)){
                            ...
                            // cek tagihan
                            $cektagihan = mysqli_num_rows(mysqli_query($koneksi,"SELECT * FROM `tbtagihan` WHERE `IdPasienrj` = '$idpasienrj' AND IdTindakan = '$data[IdTindakan]' AND StatusBayar = '1'"));
                        ?>
                            <tr>
                                <td align="center"><?php echo $no;?></td>
                                <td align="center"><?php echo $data['TanggalPeriksa'];?></td>                  
                                <td align="left"><?php echo $data['NomorSurat'];?></td>                  
                                <td align="left"><?php echo $data['StatusPemeriksaan'];?></td>                  
                                <td align="left"><?php echo $data['NamaDokter'];?></td>                  
                                <td align="left">
                                    <?php if($cektagihan == 0){?>
                                    <a onClick="return confirm('Data ingin dihapus...?')" href="pemeriksaan_mata_hapus.php?idrj=<?php echo $dtpasien;?>&idpsn=<?php echo $idpasien;?>" class="btn btn-sm btn-round btn-danger"><i class="fas fa-trash-alt"></i></a>
                                    <a href="cetak_surat_mata.php?idrj=<?php echo $dtpasien;?>&idpsn=<?php echo $idpasien;?>" class="btn btn-sm btn-round btn-info" target="_blank"><i class="fas fa-print"></i></a>
                                    <?php }?>
                                </td>
                            </tr>
                        <?php
                        ...
                        $min = $_GET['h'] - 4;
                            if($i <= $max && $i >= $min){
                                if($_GET['h'] == $i){
                                    ...
                                    echo "<li><a href='?page=pemeriksaan_mata&tgl=$tgl&nama=$key&asuransi=$_GET[asuransi]&h=$i'>$i</a></li>";
                                }
                            }
                        }
                    ?>	
...
$('.btnmodalnomorsurat').click(function(){
    ...
    // alert(idpasienrj);
    $.post( "get_nomorsurat_mata.php", { id: idpasienrj })
    .done(function( data ) {
