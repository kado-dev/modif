						...
						<div class="row">	
							...
							<div class="col-xl-2">
								<select name="status" class="form-control">
									...
									<option value="Rujuk Internal" <?php if($_GET['status'] == 'Rujuk Internal'){echo "SELECTED";}?>>Rujuk Internal</option>
								</select>
							</div>
							<div class="col-xl-3">
		...
		<div class="col-sm-12">
			<div class="table-responsive">
				<table class="table-judul">
					<thead>
						<?php if($_GET['status'] == 'Antri' OR $_GET['status'] == 'Rujuk Internal'){ ?>
						<tr>
							<th width="3%">NO.</th>
					...
					if($_GET['orderby'] == '' or $_GET['sort'] == ''){
						if($pel == "POLI LABORATORIUM"){	
							$orderbys = "GROUP BY NoRegistrasi ORDER BY date(TanggalRegistrasi) ASC, IdPasienrj ASC"; 
						}else{
							$orderbys = "GROUP BY NoRegistrasi ORDER BY date(TanggalRegistrasi) ASC, IdPasienrj ASC";
						}	
					}else{
					...
					if($status == 'Antri'){		
						$str = "SELECT *
						FROM `$tbpasienrj`  
						WHERE ".$tgl_str." AND (StatusPelayanan = 'Antri' OR StatusPelayanan = 'Proses') AND `AsalPasien`='10' AND PoliPertama='$pel'".$nama_str;
							...
							<td style="text-align:left;">
								...
								<?php
									...
									?>
										...
										?>
										<a href="?page=cetak_rujukan_bpjs&idrj=<?php echo $data['IdPasienrj'];?>&pelayanan=<?php echo $pel;?>" class="btn btn-round btn-info"> CETAK</a>
									<?php 
										}
									}else{ 
									?>	
					...
					// Rujuk Internal
						...
						WHERE SUBSTRING(NoRujukan,1,11) = '$kodepuskesmas' AND date(TanggalRujukan) = '$tglperiksa' AND `PoliRujukan`='$pel' AND `StatusPemeriksaan`='Rujuk Internal'".$nama_str;
						...
						<tr class="btnprint" data-pel="<?php echo $data['PoliPertama'];?>" data-noreg="<?php echo $data['NoRegistrasi'];?>" data-idrj="<?php echo $data['IdPasienrj'];?>">
							...
							<td style="text-align:center;"><?php echo substr($datapasienrj['NoIndex'],14);?></td>
							<td style="text-align:left;"><?php echo $data['NamaPasien']."<br/>";?></td>
							...
							<td style="text-align:center;"><?php echo $data['StatusPemeriksaan'];?></td>							
							...
							<td style="text-align:center;">
								<!-- <a href="?page=poli_periksa&id=<?php echo $data['IdPasienrj'];?>&idps=<?php echo $data['IdPasien'];?>&pelayanan=<?php echo $pel;?>&status=<?php echo $status;?>&tptgl=<?php echo $tglskrg;?>&cb=<?php echo $data['Asuransi'];?>" class="btn btn-round btn-success"><i class="fa fa-user-md (alias) faicon"></i></a> -->
								<a href="?page=poli_periksa_edit&id=<?php echo $datapasienrj['IdPasienrj'];?>&idps=<?php echo $datapasienrj['IdPasien'];?>&pelayanan=<?php echo $pel;?>&cb=<?php echo $datapasienrj['Asuransi'];?>" class="btn btn-round btn-success"> <i class="fa fa-user-md (alias) faicon"></i></a>
							</td>
						</tr>
					<?php
						} // akhir rujuk internal
					?>
					...
					?>
						<tr>
							...
							<td align="center"><?php echo $datapel['TanggalPeriksa'];?></td>
							<td align="left">
								<?php
								...
								<?php if($dtpasienrj['IdKunjunganSatuSehat'] !=''){ ?>
									<span class="btn btn-sm btn-round btn-success"><?php echo substr($dtpasienrj['IdKunjunganSatuSehat'],0,6)."xxx";?></span>
								<?php }else{ ?>
									<a href="kirim_reg_encounter.php?idrj=<?php echo $dtpasienrj['IdPasienrj'];?>&nikps=<?php echo $dtpasienrj['Nik'];?>&tgl=<?php echo date('d-m-Y', strtotime($dtpasienrj['TanggalRegistrasi']));?>&page=poli&pelayanan=<?php echo $pel;?>" class="btn btn-sm btn-round btn-info">Encounter</a>
								<?php } ?><br/>
								<?php									
									// cek tindakan
									...
									if($cektindakan > 0){
								?>
									<span class="btn btn-sm btn-round btn-warning"><?php echo "Tindakan";?></span>
								<?php 
									}
							...
							<td align="left">
								...
								<?php if($dtpasienrj['IdObservationSatuSehat'] !=''){ ?>
									<span class="btn btn-sm btn-round btn-success"><?php echo substr($dtpasienrj['IdObservationSatuSehat'],0,6)."xxx";?></span>
								<?php }else{ ?>
									<span class="btn btn-sm btn-round btn-info"><?php echo "Observation";?></span>
								<?php } ?><br/>
							</td>
							<td align="left">
								<?php
								...
								?>	
									...
									<?php if($dtpasienrj['IdConditionSatuSehat'] !=''){ ?>
										<span class="btn btn-sm btn-round btn-success"><?php echo substr($dtpasienrj['IdConditionSatuSehat'],0,6)."xxx";?></span>
									<?php }else{ ?>
										<a href="kirim_reg_condition.php?idrj=<?php echo $dtpasienrj['IdPasienrj'];?>&nikps=<?php echo $dtpasienrj['Nik'];?>&tgl=<?php echo date('d-m-Y', strtotime($dtpasienrj['TanggalRegistrasi']));?>&page=poli&pelayanan=<?php echo $pel;?>" class="btn btn-sm btn-round btn-info">Condition</a>
									<?php } ?><br/>
								<?php
									echo "Tindakan : <br/>";
