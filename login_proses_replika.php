...
$pegawai = mysqli_fetch_array(mysqli_query($koneksi,"SELECT `Otoritas` FROM `tbpegawai` WHERE NamaPegawai='$user' and Password='$pass'"));
if($pegawai['Otoritas'] == 'PROGRAMMER'){
	$str = "SELECT * FROM tbpegawai WHERE NamaPegawai='$user' and Password='$pass'";
}else{
	$str = "SELECT * FROM tbpegawai WHERE KodePuskesmas = '$kodepuskesmas' AND NamaPegawai='$user' and Password='$pass'";	
}
if(preg_match("/[']/",$user) || preg_match("/[']/",$pass)){
	...
	// die();
		...
		echo "window.location='indexawal_replika.php';";
		echo "</script>";
	}	
}
