...
/**
 * Controller untuk API P2P Jiwa
 * Endpoint: /index.php/p2pjiwa/index/{kode_puskesmas}/{tahun}/{bulan}
 * Method: GET
 * 
 * Mengembalikan data kasus Jiwa (F20, F32, F91, F52, F48, F41, F29, F45) per golongan umur dan jenis kelamin
 */
class P2pjiwa extends REST_Controller {
	...
	public function index_get(){
		try {
			...
			$kodedgs = " AND (`KodeDiagnosa` like '%F20%' OR `KodeDiagnosa` like '%F32%' OR `KodeDiagnosa` like '%F91%' OR `KodeDiagnosa` like '%F52%' OR `KodeDiagnosa` like '%F48%' OR `KodeDiagnosa` like '%F41%' OR `KodeDiagnosa` like '%F29%' OR `KodeDiagnosa` like '%F45%')";
			...
			log_message('error', 'P2P Jiwa API Error: ' . $e->getMessage());
			$response_data = [
				'Pesan' => [
