...
if(in_array("PROGRAMMER", $otoritas) || in_array("APOTEK", $otoritas)){
	...
	if($query){	
		...
		echo "document.location.href='?page=apotik_pelayanan_resep_manual_lihat_offline&norsp=$noresep&noid=$noid&statusloket=$statusloket'";
		echo "</script>";
	}else{
		echo "<script>";
		echo "alert('Data gagal dihapus');";
		echo "document.location.href='?page=apotik_pelayanan_resep_manual_lihat_offline&norsp=$noresep'";
		echo "</script>";
	} 
}else{
		echo "<script>";
