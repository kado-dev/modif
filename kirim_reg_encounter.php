<?php
	...
	$tgl = $_GET['tgl'];
	$tgl_format = date('Y-m-d', strtotime($_GET['tgl']));
    ...
    if($_GET['page']=='poli'){
        ...
        $page = "satusehat_encounter_export";
    }
	...
	if(mysqli_num_rows($getnikDokter) > 0){
        ...
        echo "<script>";
        if($hal != ''){
            echo "document.location.href='index.php?page=$page&tgl=".$tgl."&h=".$hal.";'";
        }else{
            echo "document.location.href='index.php?page=$page&tgl=$tgl'";
        }    
        echo "</script>";
