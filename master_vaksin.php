<div class="tableborderdiv">
	<div class="row">
		<div class="col-sm-12 table-responsive">
			<h3 class="judul"><b>DATA VAKSIN</b></h3>
			<div class="formbg">				
				<form role="form">
					<div class = "row">
						<input type="hidden" name="page" value="master_vaksin"/>
						<div class="col-sm-9">
							<input type="text" name="key" class="form-control key" value="<?php echo $_GET['key'];?>" required>
						</div>
						<div class="col-sm-3">
							<button type="submit" class="btn btn-round btn-warning"><span class="fa fa-search"></span></button>
							<a href="?page=master_vaksin" class="btn btn-round btn-info"><span class="fa fa-refresh"></span></a>
							<a href="?page=master_vaksin_tambah" class="btn btn-round btn-success">Tambah</a>
						</div>
					</div>	
				</form>
			</div>
	...
	<?php
		...
		$str = "SELECT * FROM `ref_vaksin`".$keys;
			...
			$min = $_GET['h'] - 4;
				if($i <= $max && $i >= $min){
					if($_GET['h'] == $i){
						...
						echo "<li><a href='?page=master_vaksin&key=$key&h=$i'>$i</a></li>";
					}
				}
			}
		?>	
