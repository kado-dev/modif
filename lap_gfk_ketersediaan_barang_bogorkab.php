				...
				<form role="form">
					<div class = "row">
						...
						<div class="col-sm-2">
							...
							<a href="lap_gfk_ketersediaan_barang_bogorkab_excel.php?tanggal_awal=<?php echo $_GET['keydate1'];?>&tanggal_akhir=<?php echo $_GET['keydate2'];?>&namaprg=<?php echo $_GET['namaprg'];?>&key=<?php echo $_GET['key'];?>" class="btn btn-success btn-white"><span class="fa fa-file-excel"></span></a>
						</div>
					</div>
				</form>	
			</div>
					...
					<tbody>
						<?php						
							...
							while($data = mysqli_fetch_assoc($query_obat)){
								...
								JOIN `tbgfkpengeluaran` b ON a.NoFaktur = b.NoFaktur 
								WHERE a.`KodeBarang`='$kodebarang' AND a.`NoBatch`='$nobatch' AND a.`NoFakturTerima`='$nofakturterima' AND
			...
			$min = $_GET['h'] - 4;
				if($i <= $max && $i >= $min){
					if($_GET['h'] == $i){
						...
						echo "<li><a href='?page=lap_gfk_ketersediaan_barang_bogorkab&keydate1=$tanggal_awal&keydate2=$tanggal_akhir&namaprg=$namaprg&key=$key&h=$i'>$i</a></li>";
					}
				}
			}
		?>	
