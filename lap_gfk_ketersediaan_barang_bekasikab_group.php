	...
	<div class="row noprint">
		<div class="col-xs-12">
			...
			<div class="formbg">
				<form role="form">
					<div class = "row">
						<input type="hidden" name="page" value="lap_gfk_ketersediaan_barang_bekasikab_group"/>
						<div class="col-sm-2">
						...
						<div class="col-sm-2">
							...
							<a href="?page=lap_gfk_ketersediaan_barang_bekasikab_group" class="btn btn-info btn-white"><span class="fa fa-refresh"></span></a>
							<a href="lap_gfk_ketersediaan_barang_bekasikab_group_excel.php?keydate1=<?php echo $_GET['keydate1'];?>&keydate2=<?php echo $_GET['keydate2'];?>&namaprg=<?php echo $_GET['namaprg'];?>&key=<?php echo $_GET['key'];?>" class="btn btn-success btn-white"><span class="fa fa-file-excel"></span></a>
						</div>
					</div>
				</form>	
			</div>
				...
				<table class="table-judul-laporan" width="100%">
					<thead>
						<tr>
							...
							<th width="9%">Expire</td>
							<th width="8%">Sumber Anggaran</td>
							<th width="8%">Tahun</td>
							<th width="11%">Supplier</td>
					...
					<tbody style="font-size: 12px;">
						<?php						
							...
							while($data = mysqli_fetch_assoc($query_obat)){
								...
								WHERE a.`KodeBarang`='$kodebarang' AND a.`NamaProgram`='$namaprogram' AND YEAR(b.TanggalPenerimaan) > '2019' AND b.TanggalPenerimaan BETWEEN '$keydate1' AND '$keydate2'";
								...
								// bekasi yang dihitung diatas tahun 2019
								$strpengeluaran = "SELECT SUM(Jumlah) AS Jumlah FROM `tbgfkpengeluarandetail` a 
								JOIN `tbgfkpengeluaran` b ON a.NoFaktur = b.NoFaktur 
								WHERE a.`KodeBarang`='$kodebarang' AND a.`NamaProgram`='$namaprogram' AND YEAR(b.TanggalPengeluaran) > '2019' AND b.TanggalPengeluaran BETWEEN '$keydate1' AND '$keydate2'";
								// echo $strpengeluaran;
			...
			$min = $_GET['h'] - 4;
				if($i <= $max && $i >= $min){
					if($_GET['h'] == $i){
						...
						echo "<li><a href='?page=lap_gfk_ketersediaan_barang_bekasikab_group&keydate1=$keydate1&keydate2=$keydate2&namaprg=$namaprg&key=$key&h=$i'>$i</a></li>";
					}
				}
			}
		?>	
