<?php
	...
	if(isset($poli)){
		...
		if(mysqli_num_rows($data_antrian_poli) == 0){
			...
			$data_antrian_poli1 = mysqli_query($koneksi, "SELECT COUNT(NomorAntrianPoli) as No FROM $tbantrian_pasien WHERE PoliPertama = '$poli' AND KodePuskesmas = '$kodepuskesmas' AND DATE(WaktuAntrian) = CURDATE()");
			$dta2 = mysqli_fetch_assoc($data_antrian_poli1);
			$nomor_antrian_poli = $dta2['No'] + 1;
		}
		
		// proses insert ket tabel
		$nokartu = $_POST['nokartu'];//no kartu bpjs
    	$kodepoli = $_POST['kodepoli'];

		$str = "INSERT INTO `$tbantrian_pasien`(`WaktuAntrian`,`KodePuskesmas`, `NomorAntrian`, `NomorAntrianPoli`, `PoliPertama`,`nomorkartubpjs`,`kodepolibpjs`,`StatusDaftar`) VALUES ('$waktuantrian','$kodepuskesmas','$nomor_antrian','$nomor_antrian_poli','$poli','$nokartu','$kodepoli','OFFLINE')";
		// echo $str;
		// die();
    ...
    $kodedokter = (int)$_POST['kodedokter'];//kodedokter bpjs
    $namadokter = $_POST['namadokter'];
    $jampraktek = $_POST['jampraktek'];
    $noantrean = $nomor_antrian_poli2;
    $angkaantrean = (int)$nomor_antrian_poli;
    $keterangan = '';

    $send_bpjs = simpan_antrean_fktp($nokartu,$nik,$nohp,$kodepoli,$namapoli,$norm,$tanggalperiksa,$kodedokter,$namadokter,$jampraktek,$noantrean,$angkaantrean,$keterangan);
    // echo $send_bpjs;
    // die();
?>
		...
		<div class="printtiket" onclick="NewPrint(2)">
			...
			<div style="text-align:center; margin-top:-5px; border-top:0px solid #000">
				...
				<table border="0" align="center" style=" margin-top:-5px;">
					<tr>
						<td><span style="font-size:28px; font-weight:bold;"><?php echo $poli;?></span></td>
						<td><span style="font-size:28px; font-weight:bold;"><?php echo $nomor_antrian_poli2;?></span></td>
					</tr>
