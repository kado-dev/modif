...
<style>
	...
	.keatas{
		...
		background:green;padding:10px 6px 4px 6px;color:white;border-radius:8px;position: absolute;top:2px;right:0px
	}
</style>
<div class="antrianshtml">
	<?php
		$jml_antrian_total = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM $tbantrian_pasien WHERE date(WaktuAntrian) = '$hariini'"));
	?>			
		...
		<div class="row">
			<div class="col-md-12" style="padding-top:-5px;">
				...
				while($data = mysqli_fetch_assoc($query)){
					$jml_antrian = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM $tbantrian_pasien WHERE PoliPertama = '$data[Pelayanan]' AND date(WaktuAntrian) = '$hariini' GROUP BY NomorAntrian, NomorAntrianPoli")); 
					// $jml_antrian = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM $tbantrian_pasien WHERE PoliPertama = '$data[Pelayanan]' AND date(WaktuAntrian) = '$hariini'")); 
