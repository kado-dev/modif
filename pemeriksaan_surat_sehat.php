<?php
    session_start();
    include "config/koneksi.php";
    include "config/helper_pasienrj.php";
    $namapegawai = $_SESSION['nama_petugas'];
    $hariini = date('Y-m-d');

    // tbpuskesmas
    $dtpuskesmas = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT * FROM `tbpuskesmas` WHERE `KodePuskesmas`='$kodepuskesmas'"));
   
    if($_POST['btnsimpan'] == 'simpan'){
        include "config/helper.php";
        $idpasien = $_POST['idpasien'];
        $idpasienrj  = $_POST['idpasienrj'];
        $tanggalawal = date('Y-m-d', strtotime($_POST['tanggalawal']))." ".date('G:i:s');
        $tinggibadan = $_POST['tinggibadan'];
        $beratbadan = $_POST['beratbadan'];
        $sistole = $_POST['sistole'];
        $diastole = $_POST['diastole'];
        $golongandarah = $_POST['golongandarah'];
        $dinyatakan = $_POST['dinyatakan'];
        $keterangan = $_POST['keterangan'];   
        $namadokter = $_POST['namadokter'];	       

        // tbpasienrj
        $dataregistrasi = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT * FROM `$tbpasienrj` WHERE `IdPasienrj` = '$idpasienrj'"));	
        $pelayanan = $dataregistrasi['Poli'];       

        // tbsuratsehat
        $str = "INSERT INTO `$tbsuratsehat`(`IdPasienrj`, `IdPasien`, `TanggalPeriksa`, 
       `TinggiBadan`, `BeratBadan`, `Sistole`, `Diastole`, `GolonganDarah`, `Dinyatakan`, `Keterangan`, `NamaDokter`) 
        VALUES ('$idpasienrj','$idpasien','$tanggalawal', '$tinggibadan','$beratbadan',
        '$sistole','$diastole','$golongandarah','$dinyatakan','$keterangan','$namadokter')";
        $query = mysqli_query($koneksi,$str);
        $idsurat = mysqli_insert_id($koneksi);  
        // echo $str;
        // die();   

        // update nomorsurat
        $nomorsurat = $idsurat."/".$dtpuskesmas['NomorSuratSehat'];    
        mysqli_query($koneksi, "UPDATE $tbsuratsehat SET `NomorSurat`='$nomorsurat' WHERE `IdSuratSehat`='$idsurat'");

        if($query){
            alert_swal('sukses','Data berhasil disimpan..');
            echo "<script>";
            echo "document.location.href='index.php?page=pemeriksaan_surat_sehat&&idrj=$idpasienrj';";	
            echo "</script>";
        }else{
            alert_swal('gagal','Data gagal disimpan..');
            echo "<script>";
            echo "document.location.href='index.php?page=pemeriksaan_surat_sehat&idrj=$idpasienrj';";
            echo "</script>";
        }
    }else{

    $idpasien = $_GET['idpsn'];
    $idpasienrj = $_GET['idrj'];
    $pelayanan = $_GET['pelayanan'];
    // tbpasienrj
    $dataregistrasi = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT * FROM `$tbpasienrj` WHERE `IdPasienrj` = '$idpasienrj'"));	
    $asuransi = $dataregistrasi['Asuransi'];
?>


<div class="tableborderdiv">
    <form action="pemeriksaan_surat_sehat.php" method="post">
        <input type="hidden" name="idpasien" value="<?php echo $dataregistrasi['IdPasien'];?>">
        <input type="hidden" name="idpasienrj" value="<?php echo $dataregistrasi['IdPasienrj'];?>">
        <input type="hidden" name="pelayanan" value="<?php echo $pelayanan;?>">
        <div class = "row">
            <div class="col-sm-12">
                <div class="formbg mt-2">
                    <a href="index.php?page=suratketerangan&idrj=<?php echo $idpasienrj;?>" class="backform"><i class="fa fa-chevron-circle-left fa fa-2x"></i></a>
                    <p style="font-size: 18px; font-weight: bold;" class="judul">Surat Keterangan Sehat</p>
                    <div class="table-responsive">
                        <table class="table-konten" width="100%">
                            <tr>
                                <td class="col-sm-2">Nomor Surat</td>
                                <td class="col-sm-10">
                                    <div class="btn-group btn-block">
                                        <input type="text" name="nomorsurat" class="form-control inputan" value="<?php echo $dtpuskesmas['NomorSuratSehat'];?>" readonly>
                                        <a href="#" class="btn btn-primary btnmodalnomorsurat" data-idpasienrj="<?php echo $dataregistrasi['IdPasienrj'];?>">Nomor Surat</a>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                <td>Tanggal Awal</td>
                                <td>
                                    <input type="date" name="tanggalawal" class="form-control inputan" value="<?php echo $hariini;?>" autofocus>
                                </td>
                            </tr>
                            <tr>
                                <td>Tinggi Badan</td>
                                <td><input type="text" name="tinggibadan" class="form-control inputan"></td>
                            </tr>
                            <tr>
                                <td>Berat Badan</td>
                                <td><input type="text" name="beratbadan" class="form-control inputan"></td>
                            </tr>
                            <tr>
                                <td>Sistole</td>
                                <td><input type="text" name="sistole" class="form-control inputan"></td>
                            </tr>
                            <tr>
                                <td>Diastole</td>
                                <td><input type="text" name="diastole" class="form-control inputan"></td>
                            </tr>
                            <tr>
                                <td>Golongan Darah</td>
                                <td>
                                    <div class="input-group">
                                        <select name="golongandarah" class="form-control inputan" required>
                                            <option value="">--Pilih--</option>
                                            <option value="-">-</option>
                                            <option value="A">A</option>
                                            <option value="B">B</option>
                                            <option value="AB">AB</option>
                                            <option value="O">O</option>
                                        </select>
                                        <div class="input-group-append">
                                            <span class="input-group-text">Pilih</span>
                                        </div>
                                    </div>	
                                </td>
                            </tr>
                            <tr>
                                <td>Dinyatakan</td>
                                <td>
                                    <div class="input-group">
                                        <select name="dinyatakan" class="form-control inputan" required>
                                            <option value="">--Pilih--</option>
                                            <option value="sehat">SEHAT</option>
                                            <option value="tidak sehat">TIDAK SEHAT</option>
                                        </select>
                                        <div class="input-group-append">
                                            <span class="input-group-text">Pilih</span>
                                        </div>
                                    </div>	
                                </td>
                            </tr>
                            <tr>
                                <td>Keterangan</td>
                                <td><textarea type="text" name="keterangan" class="form-control inputan" placeholder="Misal : Menyatakan bahwa pasien tersebut sudah dapat mengikuti kegiatan belajar kembali."></textarea></td>
                            </tr>
                            <tr>
                                <td>Dokter</td>
                                <td>
                                    <select name="namadokter" class="form-control inputan" required>
                                        <option value="">--Plih Dokter--</option>
                                        <?php
                                        $query = mysqli_query($koneksi, "SELECT * FROM `tbpegawai` WHERE `KodePuskesmas`='$kodepuskesmas' AND `Status`='Dokter' ORDER BY `NamaPegawai`");
                                            while($datadokter = mysqli_fetch_assoc($query)){
                                                if($data['NamaDokter'] == $datadokter['NamaPegawai']){
                                                    echo "<option value='$datadokter[NamaPegawai]' SELECTED>$datadokter[NamaPegawai]</option>";
                                                }else{
                                                    echo "<option value='$datadokter[NamaPegawai]'>$datadokter[NamaPegawai]</option>";
                                                }
                                            }
                                            
                                        ?>
                                    </select>
                                </td>
                            </tr>
                        </table><hr/>
                        <div class="row">
                            <div class="col-sm-6">
                                <button type="submit" name="btnsimpan" value="simpan" class="btn btn-round btn-success btnsimpan"><i class="fa fa-save"></i> Simpan</button>
                            </div>
                            <div class="col-sm-6">
                                <a href="index.php?page=suratketerangan&idrj=<?php echo $idpasienrj;?>" class="btn btn-info btn-round btninfo">Kembali</a>
                            </div>
                        </div><br/>
                    </div>
                </div>
            </div>	
        </div>
    </form>

    <div class="row mt-2">
        <div class="col-sm-12">
            <p style="font-size: 18px; font-weight: bold;" class="judul">Riwayat Pemeriksaan</p>
            <div class="table-responsive">
                <table class="table-judul">
                    <thead>
                        <tr>
                            <th width="5%">No.</th>
                            <th width="15%">NomorSurat</th>
                            <th width="15%">Tanggal Periksa</th>
                            <th width="15%">Dinyatakan</th>
                            <th width="20%">Keterangan</th>
                            <th width="20%">NamaDokter</th>
                            <th width="15%">#</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        $jumlah_perpage = 20;

                        if($_GET['h']==''){
                            $mulai=0;
                        }else{
                            $mulai= $jumlah_perpage * $_GET['h'] - $jumlah_perpage;
                        }
                    
                        // tahap 1, pilih kunjungan  
                        $idpasien = $dataregistrasi['IdPasien'];           
                        $str = "SELECT * FROM `$tbsuratsehat` WHERE `IdPasien` = '$idpasien'";
                        $str2 = $str." ORDER BY `IdSuratSehat` DESC LIMIT $mulai,$jumlah_perpage";
                        // echo $str2;
                                    
                        if($_GET['h'] == null || $_GET['h'] == 1){
                            $no = 0;
                        }else{
                            $no = $jumlah_perpage * $_GET['h'] - $jumlah_perpage;
                        }
                        
                        $query = mysqli_query($koneksi, $str2);
                        while($data = mysqli_fetch_assoc($query)){
                            $no = $no + 1;
                            $idpasien = $data['IdPasien'];
                            $idpasienrj = $data['IdPasienrj'];
                            $idsurat = $data['IdSuratSehat'];
                        ?>
                            <tr>
                                <td align="center"><?php echo $no;?></td>
                                <td align="left"><?php echo $data['NomorSurat'];?></td>
                                <td align="center"><?php echo $data['TanggalPeriksa'];?></td>
                                <td align="left"><?php echo $data['Dinyatakan'];?></td>                  
                                <td align="left"><?php echo $data['Keterangan'];?></td>                  
                                <td align="left"><?php echo $data['NamaDokter'];?></td>                  
                                <td align="center">
                                    <a onClick="return confirm('Data ingin dihapus...?')" href="pemeriksaan_surat_sehat_hapus.php?idrj=<?php echo $idpasienrj;?>&idsurat=<?php echo $idsurat;?>" class="btn btn-sm btn-round btn-danger"><i class="fas fa-trash-alt"></i></a>
                                    <a href="cetak_surat_sehat.php?idrj=<?php echo $idpasienrj;?>&idpsn=<?php echo $idpasien;?>" class="btn btn-sm btn-round btn-info" target="_blank"><i class="fas fa-print"></i></a>
                                </td>
                            </tr>
                        <?php
                        }
                        ?>
                    </tbody>
                </table>
                <ul class="pagination mt-4 noprint">
                    <?php
                        $query2 = mysqli_query($koneksi,$str);
                        $jumlah_query = mysqli_num_rows($query2);
                        
                        if(($jumlah_query % $jumlah_perpage) > 0){
                            $jumlah = ($jumlah_query / $jumlah_perpage)+1;
                        }else{
                            $jumlah = $jumlah_query / $jumlah_perpage;
                        }
                        for ($i=1;$i<=$jumlah;$i++){
                        $max = $_GET['h'] + 5;
                        $min = $_GET['h'] - 4;
                            if($i <= $max && $i >= $min){
                                if($_GET['h'] == $i){
                                    echo "<li class='active'><span class='current'>$i</span></li>";
                                }else{
                                    echo "<li><a href='?page=pemeriksaan_surat_sehat&tgl=$tgl&nama=$key&asuransi=$_GET[asuransi]&h=$i'>$i</a></li>";
                                }
                            }
                        }
                    ?>	
                </ul>
            </div>
        </div>
    </div>
    <?php
        }
    ?>
</div>

<!-- Modal tombol cetak -->
<div class="hasilmodal"></div>

<script src="assets/js/jquery-2.1.4.min.js"></script>	
<script>
$('.btnmodalnomorsurat').click(function(){
    var idpasienrj = $(this).data("idpasienrj");
    // alert(idpasienrj);	
    $.post( "get_nomorsurat_sehat.php", { id: idpasienrj })
    .done(function( data ) {
        $( ".hasilmodal" ).html( data );
        $('#modaleditdata').modal('show');
    });	
});
</script>
