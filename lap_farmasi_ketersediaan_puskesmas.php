				...
				<form role="form">
					<div class = "row">
						...
						<div class="col-sm-2">
							...
							<a href="lap_farmasi_ketersediaan_puskesmas_excel.php?bulanawal=<?php echo $_GET['bulanawal'];?>&bulanakhir=<?php echo $_GET['bulanakhir'];?>&tahun=<?php echo $_GET['tahun'];?>&namaprg=<?php echo $_GET['namaprg'];?>&key=<?php echo $_GET['key'];?>" class="btn btn-sm btn-success"><span class="fa fa-file-excel"></span></a>
						</div>
					</div>
				</form>	
			</div>
			...
			$min = $_GET['h'] - 4;
				if($i <= $max && $i >= $min){
					if($_GET['h'] == $i){
						...
						echo "<li><a href='?page=lap_farmasi_ketersediaan_puskesmas&bulanawal=$bulanawal&bulanakhir=$bulanakhir&tahun=$tahun&namaprg=$namaprg&key=$key&h=$i'>$i</a></li>";
					}
				}
			}
		?>	
