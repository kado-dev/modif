<?php
	...
	$data_medis = get_data_dokter_antrean_fktp('008',$tglsekarang);
	...
	if($list != ''){
		...
		$str2 = "DELETE FROM tbpegawaibpjsjadwal Where `kdpuskesmas` = '$kodepuskesmas' AND `kodepoli`='008'";
		...
		foreach($list as $ls){
			...
			$str = "INSERT INTO `tbpegawaibpjsjadwal`(`kdpuskesmas`,`kodepoli`,`namadokter`,`kodedokter`,`jampraktek`,`kapasitas`) VALUES ('$kdpuskesmas','008','$namadokter','$kddokter','$jampraktek','$kapasitas')";
		...
		setcookie("alert","<div class='alert alert-info'>Tidak ada data ditemukan, <br/> cek aplikasi HFIS Pastikan sudah Approve oleh BPJS</div>",time()+5);
		header('location:index.php?page=bpjs_dokter_jadwal');
	}
?>