	...
	<div class="row noprint">
		<div class="col-xs-12">
			...
			<div class="formbg">
				<form role="form">
					<div class = "row">
						<input type="hidden" name="page" value="lap_gfk_ketersediaan_barang_bogorkab_group_view_puskesmas_vaksin"/>
						<div class="col-sm-2">
						...
						<div class="col-sm-2">
							...
							<a href="?page=lap_gfk_ketersediaan_barang_bogorkab_group_view_puskesmas_vaksin" class="btn btn-sm btn-info"><span class="fa fa-refresh"></span></a>
						</div>
					</div>
				</form>	
			</div>
					...
					<tbody>
						<?php						
							...
							while($data = mysqli_fetch_assoc($query_obat)){
								...
								$strstokawal = "SELECT SUM(Stok) AS Stok FROM `tbstokawalmaster_gudang_vaksin` WHERE `KodeBarang`='$kodebarang' AND `Tahun`='$tahunlalu'";
								// echo $str_stokawal;
								$dt_stokawal_dtl = mysqli_fetch_assoc(mysqli_query($koneksi, $strstokawal));
			...
			$min = $_GET['h'] - 4;
				if($i <= $max && $i >= $min){
					if($_GET['h'] == $i){
						...
						echo "<li><a href='?page=lap_gfk_ketersediaan_barang_bogorkab_group_view_puskesmas_vaksin&keydate1=$keydate1&keydate2=$keydate2&key=$key&h=$i'>$i</a></li>";
					}
				}
			}
		?>	
