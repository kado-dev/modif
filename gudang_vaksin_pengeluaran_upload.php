<?php
	...
	$str = "UPDATE `tbgfk_vaksin_pengeluaran` SET `ImageDok`='$namaimg' WHERE `IdDistribusi`='$id'";
	$query=mysqli_query($koneksi,$str);
	// echo var_dump($str);
	// die();
	if($_POST['namalama'] != ''){
		unlink("image/dokumen_pengeluaran_vaksin/".$_POST['namalama']);
	}
	if($query){	
		if($nama_file != ''){
		copy($tmp,'image/dokumen_pengeluaran_vaksin/'.$namaimg);//proses copy
		}
		echo "<script>";
